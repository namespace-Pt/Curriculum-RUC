...
# define INPUT "data/test.in"
// # define INPUT "lexer.cpp"
...
void parse_keyword_and_identifier(ifstream &fin, ofstream &fout){
    ...
    if (SEPARATORS.find(ch) != SEPARATORS.end() || OPERATORS.find(s+ch) != OPERATORS.end() || isIn(ch," \n")){
        fin.putback(ch);
    }
    else{
...
void parse_operator(ifstream &fin, ofstream &fout){
    ...
    switch (ch)
    {
        case '=':
            getChar(fin,ch);
            if (isDigit(ch) || isLetter(ch) || isIn(ch," ")){
...
int main(int argc, char const *argv[])
{
    ...
    fout.open("result.out",ofstream::out);
    identifiers.open("identifiers.txt",ofstream::out);
    consts.open("consts.txt",ofstream::out);
    ...
    printMap(IDENTIFIERS, &identifiers);
    printMap(CONSTS, &consts);
    fin.close();
    fout.close();
