...
- 求$\mathsf{FirstVT}(P)$的算法
  ...
  - 如果有产生式$P\rightarrow Q\cdots$, 则$\forall a\in \mathsf{FirstVT}(Q), a\in \mathsf{FirstVT}(P)$
### 尾符号集LastVT
...
- 求$\mathsf{LastVT}(P)$的算法
  ...
  - 如果有产生式$P\rightarrow \cdots Q$, 则$\forall a\in \mathsf{LastVT}(Q), a\in \mathsf{LastVT}(P)$
### 计算优先级
