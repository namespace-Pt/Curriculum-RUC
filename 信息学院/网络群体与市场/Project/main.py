    ...
    while(1):
        ...
        if x == 'exit':
            ...
            if mode == '1':
                ...
                print("calculating Clustering Coefficient...")
                coefficient = graph.run("CALL gds.localClusteringCoefficient.stream('sparse') YIELD nodeId, localClusteringCoefficient RETURN gds.util.asNode(nodeId).name AS name, localClusteringCoefficient ORDER BY localClusteringCoefficient DESC LIMIT 10")
            else:
                print("You are KIDDING ME!")
                continue
